...
mod test {
    ...
    impl CryptoUpcast for CryptoCase {
        fn upcast_crypto_values(&mut self, ctx: &CryptoCtx) -> Result<CryptoCtx, Error> {
            ...
            self.hpke_out.upcast_crypto_values(ctx)?;
            Ok(*ctx)
        }
    }
    ...
    struct CryptoTestVectors {
        ...
        #[serde(rename = "hpke_plaintext__bound_u8")]
        hpke_plaintext: Vec<u8>,
    ...
    fn official_crypto_kat() {
        ...
        // Make sure the decryption of the hpke ciphertext is indeed the given plaintext
        let derived_plaintext =
            hpke::decrypt(cs, &recip_secret_key, case1.hpke_out.clone()).unwrap();
        let expected_plaintext = test_vec.hpke_plaintext.clone();
        assert_eq!(&derived_plaintext, &expected_plaintext);

        // Now make sure that we get the same hpke ciphertext if we use the same ephemeral private
        ...
        let (_, sender_secret_key) = {
            ...
            let key_material =
                [recip_public_key.as_bytes(), test_vec.hpke_plaintext.as_slice()].concat();
            // key_pair = Derive-Key-Pair(key_material)
            cs.derive_key_pair(&key_material).unwrap()
        };
        // ciphertext = Hpke-Encrypt_(sender_secret_key,recip_public_key)(plaintext)
        let derived_ciphertext = hpke::encrypt_with_scalar(
            cs,
            &recip_public_key,
            test_vec.hpke_plaintext,
            sender_secret_key,
        )
        .unwrap();
        // Now serialize both ciphertexts and make sure they agree
        assert_serialized_eq!(derived_ciphertext, case1.hpke_out);
    }
}
